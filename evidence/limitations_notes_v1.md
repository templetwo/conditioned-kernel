...
13. Verbatim board excerpts are preserved under `evidence/board_excerpts/` with a per-file hash manifest, so the correspondence is auditable without a live chronicle. **Re-exported by `evidence/export_board_excerpts.py` at every phase close and before any external review or deposit** — the archive is a snapshot, and the first one silently fell ~500 entries behind because nothing said when to refresh it. Selection is by domain query, never a hand-picked id list, for the same reason. It is expected to lag the live board by the entries posted since the last run; the trigger is phase boundaries, not continuous chasing.
...
17. **The canary is a kernel of the pilot, not an evidence-level device** (Anthony, 2026-08-05; `evidence/SUPERSESSION-002.md`). LN-2A is cited as the cause: every kernel in the frozen five is conventional, and on conventional kernels "the ECS pinned it" and "convention pinned it" make the same prediction, so no quantity of additional data on those five separates them. As an evidence-level device the canary could not measure generators; as kernel six it can. Both the frozen-five and six-kernel analyses are reported, so the added kernel can never be the thing that makes a result.
18. **Every outcome set carries an explicit cannot-evaluate class** (Anthony, 2026-08-05, elevating LN-7 to standing policy; SPEC §7a.2b). It applies to every component that reports an outcome, not only the gate chain; it may not be satisfied in prose while the return type admits two states; it never shares an encoding with success or with a declared exemption; it carries its cause, because the fail-closed-versus-infra triage is decided by cause; and every consumer handles all three classes explicitly, since a consumer that branches on two reintroduces the defect one layer up.
19. **P2 closes only when a rejection receipt exists for all three of gate 6's caps** (Anthony, 2026-08-05; SPEC §13a item 6). Superseding standing consequence 14's reporting rule with a gating one: it is no longer sufficient to decline to generalise gate 6's rejection evidence — the missing evidence must be produced before the phase closes.
